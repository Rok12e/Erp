...
{
    public partial class frmReceivedMaterial : Form
    {
        decimal invId;
        ...
        public frmReceivedMaterial(frmViewProjectPlanning _master, int _id,int _tenderId,int _planningId)
        {
            InitializeComponent();
            ...
            if (id != 0)
                this.Text = "Received Material - Edit";
            else
                this.Text = "Received Material - New";
            headerUC1.FormText = this.Text;
        }
        private void frmReceivedMaterial_Load(object sender, EventArgs e)
        {
        ...
        {
            MySqlDataReader reader = DBClass.ExecuteReader(" select * from tbl_project_material_requests where planning_id = @id and ReceivedQty > 0",
                DBClass.CreateParameter("id", id));
            if (reader.Read())
            {
                invId = id;
                dtInv.Value = DateTime.Parse(reader["ReceivedDate"].ToString());
            ...
            for (int i = 0; i < dgvItems.Rows.Count - 1; i++)
            {
                if (dgvItems.Rows[i].Cells["name"].Value != null)
                {
                    DBClass.ExecuteNonQuery(@"UPDATE tbl_project_material_requests 
                                    SET ReceivedDate = @date, ReceivedQty=@qty WHERE planning_id =@planningId and tender_id=@tenderId and itemId=@itemId",
                    ...
                    Utilities.LogAudit(frmLogin.userId, "Update Received Material", "tbl_project_material_requests", planningId, "Updated Received Material: " + dgvItems.Rows[i].Cells["name"].Value.ToString() + " Qty: " + dgvItems.Rows[i].Cells["qty"].Value.ToString());
                }
            }
        ...
        private bool insertData()
        {
            ...
            //{
            //    if (dgvItems.Rows[i].Cells["name"].Value !=null) {
            //        DBClass.ExecuteNonQuery(@"INSERT INTO tbl_project_received_material(ref_id, date, itemId, unit,qty) VALUES (@refId,@date, @itemId,@unit, @qty)",
