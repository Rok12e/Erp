...
namespace YamyProject
{
    public partial class frmAddQuotation : Form
    {
        ...
        public frmAddQuotation(int _id = 0, string _formType = "")
        {
            InitializeComponent();
        ...
        private void frmAddQuotation_FormClosing(object sender, FormClosingEventArgs e)
        {
            EventHub.Customer -= customerUpdatedHandler;
            EventHub.wareHouse -= warehouseUpdatedHandler;
        }
        ...
        private void frmAddQuotation_Load(object sender, EventArgs e)
        {
            ...
            if (formType == "")
            {
                if (id != 0)
                {
                    btnSave.Enabled = UserPermissions.canEdit("Quotation");
                    BindInvoice();
                }
            }
            else
            {
                if (id == 0 && formType == "Proforma")
                {
                    loadSalesProformaData();
                }
            }
        }
        private void loadSalesProformaData()
        {
            using (MySqlDataReader reader = DBClass.ExecuteReader(" select * from tbl_sales_proforma where id = @id",
                ...
                if (reader.Read())
                {
                    ...
                    dgvItems.Rows.Clear();
                    MySqlDataReader readerDetails = DBClass.ExecuteReader(@"SELECT tbl_sales_proforma_details.*,tbl_items.type,method, tbl_items.code ,tbl_items.name FROM tbl_sales_proforma_details INNER JOIN 
        ...
        {
            dgvItems.Rows.Clear();
            using (MySqlDataReader reader = DBClass.ExecuteReader(@"SELECT tbl_sales_quotation_details.*,tbl_items.type,method, tbl_items.code , tbl_items.name FROM tbl_sales_quotation_details INNER JOIN 
        ...
        {
            string newCode = "QU-0001";

            using (var reader = DBClass.ExecuteReader("SELECT MAX(CAST(SUBSTRING(invoice_id, 4) AS UNSIGNED)) AS lastCode FROM tbl_sales_quotation"))
            {
                if (reader.Read() && reader["lastCode"] != DBNull.Value)
                {
                    int code = int.Parse(reader["lastCode"].ToString()) + 1;
                    newCode = "QU-" + code.ToString("D4");
                }
            }
        ...
        private string GenerateNextSalesId()
        {
            ...
            using (var reader = DBClass.ExecuteReader("SELECT MAX(id) AS lastCode FROM tbl_sales_quotation"))
            {
                if (reader.Read() && reader["lastCode"] != DBNull.Value)
                {
                    int code = int.Parse(reader["lastCode"].ToString()) + 1;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Can't Save Empty Quotation Invoice");
            ...
            if (id == 0)
            {
                if (insertInvoice())
                {
                    ...
                    MessageBox.Show("The Quotation Invoice  Saved");
                    dgvItems.Rows.Clear();
        ...
        private bool updateInvoice()
        {
            ...
            DBClass.ExecuteNonQuery(@"UPDATE tbl_sales_quotation 
            ...
            DBClass.ExecuteNonQuery("delete from tbl_sales_quotation_details where sales_id =@id", DBClass.CreateParameter("id", id));
            insertInvItems();
            Utilities.LogAudit(frmLogin.userId, "Update Quotation", "Quotation", id, "Updated Quotation: " + txtNextCode.Text);
        ...
        private bool insertInvoice()
        {
            ...
            invId = decimal.Parse(DBClass.ExecuteScalar(@"INSERT INTO tbl_sales_quotation (date, customer_id, invoice_id,warehouse_id, po_num, bill_to,city,sales_man,
            ...
            Utilities.LogAudit(frmLogin.userId, "Add Quotation", "Quotation", (int)invId, "Added Quotation: " + txtNextCode.Text);
        ...
        private void insertInvItems()
        {
            for (int i = 0; i < dgvItems.Rows.Count - 1; i++)
            {
                ...
                DBClass.ExecuteNonQuery(@"
                    INSERT INTO tbl_sales_quotation_details (sales_id, item_id, qty, cost_price, price, vatp, vat, total)
            ...
            if (dgvItems.Rows.Count > 1)
            {
                ...
                else if (e.ColumnIndex == dgvItems.Columns["qty"].Index)
                {
                    ...
                    if (id != 0)
                    {
                        ...
                        using (MySqlDataReader reader = DBClass.ExecuteReader("select * from tbl_sales_quotation_details where sales_id = @sales_id and item_id = @item_id",
        ...
        private void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            string query = "select id from tbl_sales_quotation where state = 0 and id =@id";
        ...
        {
            frmLogin.frmMain.openChildForm(new frmSalesByCustomerDetails(int.Parse(cmbCustomer.SelectedValue.ToString()), "tbl_sales_quotation"));
        }
        ...
        private void guna2TileButton21_Click(object sender, EventArgs e)
        {
            ...
            Utilities.LogAudit(frmLogin.userId, "Delete Quotation", "Quotation", id, "Deleted Quotation: " + txtNextCode.Text);
            id = 0;
            clearData();
        }
