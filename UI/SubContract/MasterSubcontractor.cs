...
{
    public partial class MasterSubcontractor : Form
    {
        private DataView _dataView;
        private EventHandler SubContractUpdatedHandler;
        private EventHandler InvoiceUpdatedHandler;
        private EventHandler PaymentVoucherHandler;

        public MasterSubcontractor()
        {
            InitializeComponent();
            LocalizationManager.LocalizeForm(this);
            SubContractUpdatedHandler = PaymentVoucherHandler = InvoiceUpdatedHandler = (sender, args) => BindSubContract();
            EventHub.SubContract += SubContractUpdatedHandler;
            EventHub.PurchaseInv += InvoiceUpdatedHandler;
            EventHub.PaymentVoucher += PaymentVoucherHandler;
            guna2HtmlLabel2.Text = Text;
        }
        private void MasterSubcontractor_FormClosing(object sender, FormClosingEventArgs e)
        {
            EventHub.PurchaseInv -= InvoiceUpdatedHandler;
            EventHub.SubContract -= SubContractUpdatedHandler;
            EventHub.PaymentVoucher -= PaymentVoucherHandler;
        ...
        private void MasterSubcontractor_Load(object sender, EventArgs e)
        {
        ...
        public void BindSubContract()
        {
            string query = @"SELECT c.id, 
                    ...
                    LEFT JOIN tbl_vendor_category tc ON c.Cat_id = tc.id WHERE c.type='Subcontractor' ";
            if (cmbState.Text == "Active Subcontractor")
                query += " AND c.active = 0";
            else if (cmbState.Text == "Inactive Subcontractor")
                query += " AND c.active != 0";
            ...
            {
                BindSubContractInvoice();
                //editCustomerToolStripMenuItem1.Visible = UserPermissions.canEdit("SubContract Center");
                //deleteSubContractToolStripMenuItem.Visible = UserPermissions.canDelete("SubContract Center");
            }
            else
                ClearSubContractDetails();
        ...
        private void BindSubContractInvoice()
        {
            BindCustomerInfo();
            BindInvoice();
            BindCashInvoice();
        ...
        private void BindCashInvoice()
        {
            ...
            string query = @"SELECT t.id, 
                        ...
                        t.Type, 
                        CASE 
                            WHEN t.type = 'Purchase Invoice Cash0' THEN IF(t.debit = 0, t.credit, t.debit)
                            ELSE NULL 
                        END AS `Amount`
        ...
        private void ClearSubContractDetails()
        {
        ...
        private void dgvInvoice_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.RowIndex < dgvTransactions.Rows.Count)
            {
                if (dgvTransactions.Columns.Contains("type"))
                {
                    ...
                    else if (CommonInsert.GetFormName(dgvTransactions.Rows[e.RowIndex].Cells["type"].Value.ToString()) == "Subcontractor Opening Balance")
                    {
                        frmLogin.frmMain.openChildForm(new frmTransactionJournal(int.Parse(dgvTransactions.Rows[e.RowIndex].Cells["InvoiceId"].Value.ToString()), "Subcontractor Opening Balance", (dgvTransactions.Rows[e.RowIndex].Cells["InvoiceId"].Value.ToString()), (dgvTransactions.Rows[e.RowIndex].Cells["Date"].Value.ToString())));
                    }
        ...
        {
            new frmViewSubcontractor(0).ShowDialog();
        }
        private void editCustomerToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (dgvCustomer.Rows.Count == 0)
                return;
            new frmViewSubcontractor(int.Parse(dgvCustomer.SelectedRows[0].Cells["id"].Value.ToString())).ShowDialog();
        }
        private void dgvCustomer_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            //BindInvoice(new List<string> { "%Purchase Invoice Cash%" }, new List<string>(), dgvInvoiceCash);
            //BindInvoice(new List<string> { "Subcontractor%", "Purchase%" ,"Check Cancel"}, new List<string> { "%Purchase Invoice Cash%" }, dgvTransactions);
            BindCustomerInfo();
            BindInvoice();
            BindCashInvoice();
            ConfigureDataGridViews();
                ...
                Dictionary<string, float> widthRatio = new Dictionary<string, float>
    {
        ...
        private void receivePaymentToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgvCustomer.Rows.Count == 0)
                return;
            frmLogin.frmMain.openChildForm(new frmViewPaymentVoucher(0, int.Parse(dgvCustomer.SelectedRows[0].Cells["id"].Value.ToString()), true));

        }

        private void deleteSubContractToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (dgvCustomer.Rows.Count == 0)
                return;
            ...
            {
                MessageBox.Show("Subcontractor has transactions. Cannot delete.", "Cannot Delete", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        private void DeleteData(int _id)
        {
            DBClass.ExecuteNonQuery(@"Delete from tbl_vendor where id = @id;Delete from tbl_transaction where transaction_id=@id and type='Subcontractor Opening Balance'",
                DBClass.CreateParameter("id", _id));
            Utilities.LogAudit(frmLogin.userId, "Subcontractor Deleted", "Subcontractor", _id, "Deleted Subcontractor with ID: " + _id);
            BindSubContract();
            MessageBox.Show("Subcontractor deleted successfully.", "Deleted", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void importToolStripMenuItem_Click(object sender, EventArgs e)
        {
            frmLogin.frmMain.openChildForm(new frmOpeningBalance("Subcontractor"));
        }
        ...
        private void toolStripMenuItemword_Click(object sender, EventArgs e)
        {
            if (dgvCustomer.CurrentRow == null)
            {
                MessageBox.Show("Please select a Subcontractor.");
                return;
            }
        ...
        private void ExportCashToWord(DataGridView dgv, string customerName, string filePath)
        {
            ...
            // Header
            var title = doc.InsertParagraph("Subcontractor Name: " + customerName)
                           .FontSize(14)
                           .Bold()
                           .SpacingAfter(20);
        ...
        private void guna2Button1_Click(object sender, EventArgs e)
        {
            if (dgvCustomer.Rows.Count == 0)
                return;
            frmLogin.frmMain.openChildForm(new frmViewSubcontractor(int.Parse(dgvCustomer.SelectedRows[0].Cells["id"].Value.ToString())));
        }
    }
}
