...
namespace YamyProject
{
    public partial class frmDebitNote : Form
    {
        ...
        decimal invId;
        int level4VendorId, level4VatId, level4PurchaseReturn, level4COGS, level4Inventory;
        int id;

        public frmDebitNote(int id=0)
        {
            InitializeComponent();
            LocalizationManager.LocalizeForm(this);
            customerUpdatedHandler = (sender, args) => BindCombos.PopulateCustomers(cmbCustomer);
            this.id = id;
            headerUC1.FormText = id == 0 ? "Debit Note - New" : "Debit Note - Edit";
        }
        ...
        private void frmDebitNote_Load(object sender, EventArgs e)
        {
            ...
            {
                BindInvoice();
                btnSave.Enabled = UserPermissions.canEdit("Debit Note");
            }
        }
        ...
        private string GenerateNextSalesCode()
        {
            ...
            using (var reader = DBClass.ExecuteReader("SELECT MAX(CAST(SUBSTRING(invoice_id, 4) AS UNSIGNED)) AS lastCode FROM tbl_debit_note"))
            {
        ...
        {
            using (MySqlDataReader reader = DBClass.ExecuteReader(" select * from tbl_debit_note where id = @id",
        ...
        private void BindInvoiceItems()
        {
            dgvItems.Rows.Clear();
            int count = 1;
            using (MySqlDataReader reader = DBClass.ExecuteReader(@"SELECT * FROM tbl_debit_note_details Where ref_id = @id;",
                                                            DBClass.CreateParameter("id", id)))
                while (reader.Read())
                {
        ...
        {
            BindCombos.PopulateVendors(cmbCustomer);
            level4VendorId = BindCombos.SelectDefaultLevelAccount("Vendor");
            level4VatId = BindCombos.SelectDefaultLevelAccount("Vat Input");
            level4Inventory = BindCombos.SelectDefaultLevelAccount("Inventory");
            level4PurchaseReturn = BindCombos.SelectDefaultLevelAccount("PurchaseReturn");
        }
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (id == 0)
            {
                if (insertInvoice())
                {
                    EventHub.RefreshDebitNote();
                    if (checkBox4.Checked == true)
                    {
                        loadPrint();
                    }
                    MessageBox.Show("The Debit Saved");
                    dgvItems.Rows.Clear();
            ...
            else
            {
                if (updateInvoice())
                {
                    ...
                    MessageBox.Show("The Debit Updated");
                    dgvItems.Rows.Clear();
                }
            }
        }
        ...
        private bool updateInvoice()
        {
            if (!chkRequiredDate())
                return false;
            DBClass.ExecuteNonQuery(@"UPDATE tbl_debit_note 
            ...
            DBClass.ExecuteNonQuery(@"DELETE FROM tbl_debit_note_details WHERE ref_id = @id", DBClass.CreateParameter("id", id));
            insertInvItems();
            CommonInsert.DeleteTransactionEntry(id, "Debit Note");
            addTransaction();
            Utilities.LogAudit(frmLogin.userId, "Update Debit Note", "Debit Note", id, "Updated Debit Note: " + invCode);
        ...
        private bool insertInvoice()
        {
            ...
            invId = decimal.Parse(DBClass.ExecuteScalar(@"INSERT INTO tbl_debit_note (date, credit_account,debit_account,type,invoice_id,
               ...
               DBClass.CreateParameter("type", "Vendor"),
               DBClass.CreateParameter("invoice_id", invCode),
               DBClass.CreateParameter("amount", txtAmount.Text),
               DBClass.CreateParameter("vat", txtVat.Text),
               DBClass.CreateParameter("total", txtTotalAmount.Text),
            ...
            addTransaction();
            Utilities.LogAudit(frmLogin.userId, "Add Debit Note", "Debit Note", (int)invId, "Added Debit Note: " + invCode);
            return true;
        }
        private void addTransaction() {
            CommonInsert.addTransactionEntry(dtInv.Value.Date, level4VendorId.ToString()
                 , "0", txtTotalAmount.Text, invId.ToString(), cmbCustomer.SelectedValue.ToString(), "Debit Note", "Debit Note", "Debit Note NO. " + invCode,
            ...
            if (decimal.Parse(txtVat.Text) > 0)
              ...
              level4VatId.ToString(), txtVat.Text, invId.ToString(), "0", "0", "Debit Note", "Debit Note",
              "Vat Input For Invoice No. " + invCode, frmLogin.userId, DateTime.Now.Date,txtNextCode.Text);
            CommonInsert.addTransactionEntry(dtInv.Value.Date,
              level4PurchaseReturn.ToString(), txtAmount.Text, "0", invId.ToString(), "0", "Debit Note", "Debit Note",
              "Revenue For Invoice No. " + invCode, frmLogin.userId, DateTime.Now.Date,txtNextCode.Text);
            ...
            //    CommonInsert.addTransactionEntry(dtInv.Value.Date,
            //         accountId,
            //          "0", TotalAmount.ToString(), invId.ToString(), cmbCustomer.SelectedValue.ToString(), cmbPaymentMethod.Text == "Credit" ? "Purchase Invoice" : "Purchase Invoice Cash", "PURCHASE", "Purchase Invoice NO. " + invCode,
            //         frmLogin.userId, DateTime.Now.Date, txtNextCode.Text);
            //}
            //if (TotalVatAmount > 0)
            //{
            ...
            foreach (DataGridViewRow row in dgvItems.Rows)
            {
                if (Convert.ToBoolean(row.Cells["select"].Value))
                {
                    ...
                    DBClass.ExecuteNonQuery(@"
                        INSERT INTO tbl_debit_note_details(ref_id, inv_no, invoice_id, invoice_date, invoice_type,total,vat,amount,balance,remaining)
                    ...
                    object result = DBClass.ExecuteScalar(
                          "SELECT SUM(`change`) FROM tbl_purchase WHERE id = @id",
                    ...
                    Utilities.LogAudit(frmLogin.userId, "Add Debit Note Item", "Debit Note", (int)invId, "Added Debit Note Item: " + _invNo + " for Invoice ID: " + _invId);
                }
            }
        }
        ...
        private bool chkRequiredDate()
        {
            ...
            if (level4VendorId == 0 | level4VatId == 0 | level4PurchaseReturn == 0 | level4COGS == 0 | level4Inventory == 0)
            {
        ...
        {
            txtAmount.Text =
                txtInvoiceId.Text = txtTotalAmount.Text = txtVat.Text = "";
        ...
        private void cmbCustomer_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            using (MySqlDataReader reader = DBClass.ExecuteReader("select * from tbl_vendor where id = " + cmbCustomer.SelectedValue.ToString()))
                if (reader.Read())
                {
        ...
        private void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            string query = "select id from tbl_debit_note where state = 0 and id =@id";
            ...
            using (MySqlDataReader reader = DBClass.ExecuteReader(query, DBClass.CreateParameter("id", cmbCustomer.SelectedValue.ToString())))
            {
                ...
                while (reader.Read())
                {
                    ...
                    dgvItems.Rows.Add(
                        ...
                        reader["id"].ToString(),
                        reader["Total With VAT"].ToString(),  // or use "net" if you change the alias
                        reader["vat"].ToString(),
        ...
        {
            return DBClass.ExecuteDataTable(@"select tbl_debit_note.* ,CASE WHEN TYPE = 'Customer' THEN (SELECT NAME FROM tbl_customer WHERE id = tbl_debit_note.debit_account) 
                                             WHEN type = 'Vedor' THEN (select Name from tbl_vendor  WHERE id = tbl_debit_note.debit_account)  ELSE '' END  AS NAME from tbl_debit_note 
                                              where id = @id;", DBClass.CreateParameter("@id", a1));
        }
        public DataTable detailsData(string a1)
        {
            return DBClass.ExecuteDataTable(@"select  * from tbl_debit_note_details where ref_id = @id;", DBClass.CreateParameter("@id", a1));
        }
        ...
        public void ShowReport()
        {
            ...
            try
            {
                ...
                // Create the report 
                //CPVDebitNote cr = new CPVDebitNote();
                string reportPath = Path.Combine(Application.StartupPath, "CrystalReport", "DebitNote.rpt");
                ...
                DataTable voucher = headerData(invId.ToString());
                DataTable details = detailsData(invId.ToString());
                if (companyData != null)  // Ensure that data was successfully retrieved
                {
                    ...
                    cr.Subreports["VoucherHeader"].SetDataSource(voucher);
                    cr.Subreports["Details"].SetDataSource(details);
                    cr.Subreports["VoucherFooter"].SetDataSource(voucher);
                }
                else
                {
