...
{
    public partial class frmViewAssignReport : Form
    {
        private string type;

        public frmViewAssignReport(string _type="")
        {
            InitializeComponent();
        ...
        private void frmViewAssignReport_Load(object sender, EventArgs e)
        {
        ...
        public void BindData()
        {
            DataTable dt;
            string query = "";
            query = @"SELECT a.id,c.code Code,c.Date,c.name,c.type Type,(SELECT NAME FROM tbl_project_role WHERE id = ROLE) ROLE,price_unit `Price / Unit`,unit_time `Default Unit/Time`,max_unit_time `Max Unit/Time` from tbl_project_activity_assignment a LEFT JOIN tbl_project_activity b ON a.activity_id = b.id Inner JOIN tbl_project_resource c ON c.id = a.resource_id 
                        AND c.employee_id > 0 ";
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvItems.Rows.Count > 0)
            {
                GeneratePDF(dgvItems, "Project Assigned");
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            if (dgvItems.Rows.Count > 0)
            {
                GenerateExcel(dgvItems, "Project Assigned");
