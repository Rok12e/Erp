...
{
    public partial class frmAddSalesOrder : Form
    {
        ...
        public frmAddSalesOrder(int id = 0)
        {
            InitializeComponent();
        ...
        private void frmAddSalesOrder_FormClosing(object sender, FormClosingEventArgs e)
        {
            EventHub.Customer -= customerUpdatedHandler;
            EventHub.wareHouse -= warehouseUpdatedHandler;
        }
        ...
        private void frmAddSalesOrder_Load(object sender, EventArgs e)
        {
            ...
            if (id !=0)
            {
                btnSave.Enabled = UserPermissions.canEdit("Sales Order");
                BindInvoice();
            }
        }
        ...
        {
            dgvItems.Rows.Clear();
            using (MySqlDataReader reader = DBClass.ExecuteReader(@"SELECT tbl_sales_order_details.*,tbl_items.type,method, tbl_items.code ,tbl_items.name FROM tbl_sales_order_details INNER JOIN 
                ...
                while (reader.Read())
                {
                    dgvItems.Rows.Add(reader["item_id"].ToString(), "", reader["code"].ToString(), reader["name"].ToString(), Utilities.FormatDecimal(reader["qty"]),
                        Utilities.FormatDecimal(reader["cost_price"]), Utilities.FormatDecimal(reader["price"]));
        ...
        {
            string newCode = "SO-0001";

            using (var reader = DBClass.ExecuteReader("SELECT MAX(CAST(SUBSTRING(invoice_id, 4) AS UNSIGNED)) AS lastCode FROM tbl_sales_order"))
            {
                if (reader.Read() && reader["lastCode"] != DBNull.Value)
                {
                    int code = int.Parse(reader["lastCode"].ToString()) + 1;
                    newCode = "SO-" + code.ToString("D4");
                }
            }
        ...
        private string GenerateNextSalesId()
        {
            ...
            using (var reader = DBClass.ExecuteReader("SELECT MAX(id) AS lastCode FROM tbl_sales_order"))
            {
                if (reader.Read() && reader["lastCode"] != DBNull.Value)
                {
                    int code = int.Parse(reader["lastCode"].ToString()) + 1;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (id == 0)
            {
                if (insertInvoice())
                {
                    ...
                    MessageBox.Show("The Sales Order Invoice  Saved");
                    dgvItems.Rows.Clear();
        ...
        private bool updateInvoice()
        {
            ...
            DBClass.ExecuteNonQuery(@"UPDATE tbl_sales_order 
            ...
            DBClass.ExecuteNonQuery("delete from tbl_sales_order_details where sales_id =@id", DBClass.CreateParameter("id", id));
            insertInvItems();
            Utilities.LogAudit(frmLogin.userId, "Update Sales Order", "Sales Order", (int)id, "Updated Sales Order: " + txtNextCode.Text);
        ...
        private bool insertInvoice()
        {
            ...
            invId = decimal.Parse(DBClass.ExecuteScalar(@"INSERT INTO tbl_sales_order (date, customer_id, invoice_id,warehouse_id, po_num, bill_to,city,sales_man,
            ...
            Utilities.LogAudit(frmLogin.userId, "Add Sales Order", "Sales Order", (int)invId, "Added Sales Order: " + txtNextCode.Text);
        ...
        private void insertInvItems()
        {
            ...
            for (int i = 0; i < dgvItems.Rows.Count - 1; i++)
            {
                ...
                valueList.Add($"(@sales_id, @item_id{paramIndex}, @qty{paramIndex}, @cost_price{paramIndex}, @price{paramIndex}, @vatp{paramIndex}, @vat{paramIndex}, @total{paramIndex})");
            ...
            string sql = $@"
                INSERT INTO tbl_sales_order_details (sales_id, item_id, qty, cost_price, price, vatp, vat, total)
            ...
            using (var conn = DBClass.GetConnection())
            {
                ...
                {
                    cmd.Parameters.AddWithValue("@sales_id", invId);
            ...
            if (dgvItems.Rows.Count > 1)
            {
                ...
                else if (e.ColumnIndex == dgvItems.Columns["qty"].Index)
                {
                    ...
                    if (id != 0)
                    {
                        ...
                        using (MySqlDataReader reader = DBClass.ExecuteReader("select * from tbl_sales_order_details where sales_id = @sales_id and item_id = @item_id",
        ...
        private void btnPrevious_Click(object sender, EventArgs e)
        {
            ...
            string query = "select id from tbl_sales_order where state = 0 and id =@id";
        ...
        {
            new frmViewCustomer(0).ShowDialog();
        }
        ...
        {
            return DBClass.ExecuteDataTable("SELECT * FROM tbl_company WHERE ID = @1", DBClass.CreateParameter("@1", id));
        }
        ...
        public DataTable SalesDetails(string a1)
        {
            return DBClass.ExecuteDataTable(@"SELECT s.id,s.date,s.customer_id,s.invoice_id,s.bill_to,s.city,s.sales_man,s.ship_date,s.ship_via,s.ship_to,(SELECT NAME FROM tbl_coa_level_4 WHERE id=s.account_cash_id) accountName,s.po_num,s.payment_method,s.payment_terms,s.payment_date,s.total,s.vat,s.net,s.pay,s.change 
                                    ,c.name customerName,c.main_phone,c.email,c.trn FROM tbl_sales_order s,tbl_customer c WHERE s.customer_id = c.id AND s.id AND s.id=@salesId;", DBClass.CreateParameter("@salesId", a1));
        }
        ...
        public void ShowReport()
        {
            try
            {
                string reportPath = Path.Combine(Application.StartupPath, "CrystalReport", "SalesOrderInvoice.rpt");
        ...
        private void guna2TileButton21_Click(object sender, EventArgs e)
        {
            ...
            Utilities.LogAudit(frmLogin.userId, "Delete Sales Order", "Sales Order", id, "Deleted Sales Order: " + id);
        ...
        {
            frmLogin.frmMain.openChildForm(new frmSalesByCustomerDetails(int.Parse(cmbCustomer.SelectedValue.ToString()), "tbl_sales_order"));
        }
