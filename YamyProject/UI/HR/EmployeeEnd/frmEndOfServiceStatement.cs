...
{
    public partial class frmEndOfServiceStatement : Form
    {
        int id;

        public frmEndOfServiceStatement(int id=0)
        {
            InitializeComponent();
        ...
        private void frmEndOfServiceStatement_Load(object sender, EventArgs e)
        {
                DataTable dt = DBClass.ExecuteDataTable(@"SELECT ROW_NUMBER() OVER (
                ORDER BY tbl_end_of_service.id) AS SN ,`code`, `name`, `Reference`, `description`,leave_days,debit,credit,(credit - debit) AS Balance
                FROM `tbl_end_of_service`
                WHERE code=@id", DBClass.CreateParameter("id", id));
