...
{
    public partial class frmViewResourceReport : Form
    {
        private string type;

        public frmViewResourceReport(string _type="")
        {
            InitializeComponent();
        ...
        private void frmViewResourceReport_Load(object sender, EventArgs e)
        {
        ...
        public void BindData()
        {
            DataTable dt;
            string query = "";
            query = @"SELECT a.id,c.code Code,c.Date,c.name Name,c.type Type,(SELECT NAME FROM tbl_project_role WHERE id = role) Role,IFNULL((SELECT NAME FROM tbl_employee WHERE id = employee_id),'') Employee,
                        price_unit `Price / Unit`,unit_time `Default Unit/Time`,max_unit_time `Max Unit/Time` 
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvItems.Rows.Count > 0)
            {
                GeneratePDF(dgvItems, "Project Resource");
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            if (dgvItems.Rows.Count > 0)
            {
                GenerateExcel(dgvItems, "Project Resource");
