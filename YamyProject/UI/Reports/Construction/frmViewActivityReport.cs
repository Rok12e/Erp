...
{
    public partial class frmViewActivityReport : Form
    {
        private string type;

        public frmViewActivityReport(string _type="")
        {
            InitializeComponent();
        ...
        private void frmViewActivityReport_Load(object sender, EventArgs e)
        {
        ...
        public void BindData()
        {
            DataTable dt;
            string query = "";
            query = @"SELECT id,(SELECT NAME FROM tbl_items_boq WHERE id = tbl_project_activity.code LIMIT 1) AS NAME,
                      code,start_date,end_date,progress,status FROM tbl_project_activity
                        WHERE id >0 "; // AND planning_id = @YourPlanningId;
            List<MySqlParameter> parameters = new List<MySqlParameter>();

            if (!chkDate.Checked)
            {
                query += " and planning_id IN (SELECT id FROM tbl_project_planning where date >= @dateFrom and date <= @dateTo)";
            }
            //else
            //{
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            if (dgvItems.Rows.Count > 0)
            {
                GeneratePDF(dgvItems, "Project Activity");
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            if (dgvItems.Rows.Count > 0)
            {
                GenerateExcel(dgvItems, "Project Activity");
