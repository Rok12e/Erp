...
{
    public partial class frmPurchaseByItemSummary : Form
    {
        public frmPurchaseByItemSummary()
        {
            InitializeComponent();
                    ...
                    else if (currLvl == 2)
                    {
                        ...
                        string sql = @"
                                     SELECT                        
                                        ...
                                        ROUND((SUM(sd.total) / (
                                            SELECT SUM(total) FROM tbl_purchase_details WHERE purchase_id IN (SELECT id FROM tbl_purchase WHERE state = 0 AND date >= @startDate AND date<= @endDate)
                                    ...
                                    WHERE     
                                        ...
                                        AND sd.purchase_id IN (SELECT id FROM tbl_purchase WHERE state = 0 AND date >= @startDate AND date<= @endDate)
                                    GROUP BY  
                                        i.id, i.name
                                    ORDER BY  
                                        i.name
        ...
        private void dgvSales_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                frmLogin.frmMain.openChildForm(new frmPurchaseByItemDetails(itemId));
            }
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            FormattedText titleText = center.AddFormattedText("Purchase By Item Summery\n", TextFormat.Bold);
            ...
            using (SaveFileDialog saveFile = new SaveFileDialog())
            {
                ...
                saveFile.FileName = "PurchaseByItemSummary.pdf";
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            using (SaveFileDialog saveDialog = new SaveFileDialog())
            {
                ...
                saveDialog.FileName = "PurchaseByItemSummary.xlsx";
        ...
        private void frmPurchaseByItemSummary_Load(object sender, EventArgs e)
        {
            btnPrint.Text = "Print";
            using (MySqlDataReader reader = DBClass.ExecuteReader("SELECT name FROM tbl_company LIMIT 1"))
                if (reader.Read())
